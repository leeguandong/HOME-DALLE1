...
import sys
sys.path.insert(0, '/home/ivms/local_disk/dalle/DALL-E-master/')
# /usr/local/python3/lib/python3.6/site-packages/taming/models
...
class OpenAIDiscreteVAE(nn.Module):
    def __init__(self):
        super().__init__()
        assert version.parse(get_pkg_version('torch')) < version.parse(
            '1.11.0'), 'torch version must be <= 1.10 in order to use OpenAI discrete vae'
        
#         import pdb;pdb.set_trace()
#         self.enc = load_model(download(OPENAI_VAE_ENCODER_PATH))
#         self.dec = load_model(download(OPENAI_VAE_DECODER_PATH))
        self.enc = load_model("/home/ivms/net_disk_project/19045845/weights/encoder.pkl")
        self.dec = load_model("/home/ivms/net_disk_project/19045845/weights/decoder.pkl")
